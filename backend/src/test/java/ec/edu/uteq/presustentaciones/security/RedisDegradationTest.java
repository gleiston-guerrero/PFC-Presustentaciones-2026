...
class RedisDegradationTest {
    ...
    void rateLimiterWithRedisDownLanzaExcepcionDeDegradationEnVezDePermitirODevolver500Crudo() {
        ...
        when(valueOps.get(anyString())).thenThrow(redisDown());
    ...
    void cacheErrorHandlerNoPropagaLaExcepcionDeLecturaDeCache() {
        ...
        assertDoesNotThrow(() -> errorHandler.handleCacheGetError(redisDown(), cache, "clave-cualquiera"));
        assertDoesNotThrow(() -> errorHandler.handleCachePutError(redisDown(), cache, "clave-cualquiera", "valor"));
        assertDoesNotThrow(() -> errorHandler.handleCacheEvictError(redisDown(), cache, "clave-cualquiera"));
        assertDoesNotThrow(() -> errorHandler.handleCacheClearError(redisDown(), cache));
    }
