 ...
 * (V19__history_minutes_y_reportes.sql). Análoga a {@link HistoryEstadosSubmission},
...
@AllArgsConstructor
@Builder
public class HistoryEstadoMinutes {
    ...
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler"})
    private EstadoMinutes estadoAnterior;

    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "estado_nuevo_id", nullable = false)
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler"})
    private EstadoMinutes estadoNuevo;
