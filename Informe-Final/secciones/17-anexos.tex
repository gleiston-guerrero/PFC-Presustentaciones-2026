...
dando un puntaje individual entre 0 y 100 (no es un porcentaje). Dos artefactos la implementan, y
conviene no confundirlos: \texttt{scripts/sus-analysis.ipynb} calcula la \emph{ronda en papel} desde
\texttt{docs/mediciones/sus/sus-respuestas.csv} (con tres casos de autotest, ejecutado de punta a punta
con \texttt{nbconvert} sin errores), y \texttt{scripts/sus-estadistica.py} calcula la \textbf{cifra de
cierre} desde el CSV sellado por el tercero, además de la corrección de Holm y el $\alpha$ de Cronbach.
La verificación automatizada (\texttt{make verify}) comprueba esta última, que es la que se reporta.

\textbf{Estado real (Sección~\ref{sec:evaluacion}):} el resultado que se reporta es el de la ronda del
2026-09-18, aplicada en un formulario alojado por un tercero que sella cada respuesta con la hora de su
propio servidor (\textbf{media 52.83/100, DE 12.06, IC 95\,\% [46.16, 59.51], $n=15$}), con
consentimiento individual explícito en las 15. De la ronda anterior en papel, también de 15 personas,
\textbf{solo 4 tienen fecha de aplicación verificable}; las otras 11 llevan una fecha escrita a mano que
no se sostiene (posterior al commit que las versiona y, en varios casos, posterior al día de hoy) y
quedan retractadas y declaradas pendientes de confirmar, no fabricadas ni descartadas --- ver
...
\midrule
33978578357 & \texttt{99a6636} & \texttt{success} & Cobertura de cierre del 2026-09-05 (63.17\,\%, 395 tests) \\
...
\bottomrule
\end{tabular}
\caption[Corridas verdes de integración continua]{Corridas verdes de integración continua
verificadas contra la API de GitHub, capturadas el \textbf{2026-09-05}. Las cifras de esa fila
(63.17\,\% de cobertura, 395 tests) son las de \emph{ese} commit, no las de cierre del informe
...
pruebas de entonces (2026-09-05) pasan y el análisis estático de seguridad corre sin romper el
pipeline; la suite creció después hasta las 804 de la corrida de cierre.
