...
La Tabla~\ref{tab:holm-bonferroni} resume el resultado de las tres pruebas tras la corrección: siguen
siendo significativas; la separación entre caché fría y
...
\begin{enumerate}[nosep]
    \item \textbf{IDOR de lectura en Evaluaciones/Anteproyectos:} \texttt{EvaluationController},
    \texttt{EvaluationPanelistController}, \texttt{RubricEvaluationController} y
    \texttt{ProposalController} exponían notas, observaciones de tribunal y el PDF del anteproyecto
    ...
    (\texttt{SubmissionAccessService}) que exige ser el estudiante dueño, un jurado o tutor asignado, o
    tener el permiso administrativo correspondiente; 403 verificado en vivo.
    \item \textbf{IDOR de escritura en registro de notas de jurado:} \texttt{EvaluationPanelistService} y
    \texttt{RubricEvaluationService} permitían que un docente-jurado registrara una nota \emph{a nombre
    ...
    y \texttt{AppUserController.create()} recibían la entidad \texttt{AppUser} completa del cliente; un
...
\begin{lstlisting}[language=Java,style=codigobase,caption={Correccion del IDOR de \texttt{TeacherController}: verificacion de propiedad del recurso antes de exponerlo.},label={lst:idor-docente}]
...
\textbf{Retractación de las 11 hojas.} Las 11 hojas con fecha no verificable quedan
\emph{retiradas} como evidencia: \textbf{no contribuyen a ninguna cifra de usabilidad reportada en este
informe} --- el resultado que se reporta, 52.83, se calcula únicamente con las 15 respuestas del
formulario del 18-sep. Aparecen en un solo lugar, la comprobación de si los puntajes del papel fueron
inventados (más abajo), que es una afirmación \emph{sobre} las hojas y no un uso de ellas como evidencia
de usabilidad; ahí van rotuladas como retractadas y nunca como resultado. Se conservan
versionadas y sin modificar, marcadas como no verificables, porque borrar evidencia señalada sería peor
que conservarla declarada. El equipo declara, y así se registra --- como declaración propia, no como
hallazgo verificado --- que durante la aplicación en papel había una fecha a la vista que varios
participantes copiaron en la casilla correspondiente en lugar de escribir la del día. Esa explicación es
compatible con las 6 hojas que comparten el 26/09/2026, pero \textbf{no cubre las otras 5}, que llevan
cuatro fechas distintas (18/09, 24/09 dos veces, 25/09 y 29/09): una sola fecha a la vista produce una
sola fecha, no cinco. El equipo no tiene explicación para esas cinco, y por eso la declaración se
registra como contexto y no como reparación --- aunque se acepte por completo, las 11 hojas siguen
retiradas. El equipo añade, preguntado expresamente por el origen, que la aplicación fue
\emph{presencial} y en las mismas condiciones que las 4 hojas de fecha verificable, y que lo que salió
mal fue únicamente la fecha escrita a mano. Esa ampliación cambia lo que se afirma que ocurrió, pero no
el tratamiento: sigue sin ser verificable, sigue sin explicar cinco fechas distintas, y las 11 hojas
siguen retiradas. Sobre quién entregó materialmente esas hojas, incluida la segunda versión con el mes
corregido, el equipo declara que fue el integrante Barreto Rosado, que no participa en esta ronda de
recuperación. Se registra porque el evaluador preguntó por un nombre, pero es una declaración de una
sola parte, no explica por qué las fechas eran cinco y posteriores al escaneo, y no traslada la
responsabilidad de haber incorporado ese material al expediente sin comprobarlas, que era del equipo.
Detalle en \texttt{docs/mediciones/sus/SUS-RESULTS.md}.


\textbf{Ronda del 2026-09-18 con fecha sellada por un tercero.} Para cerrar el defecto de forma sin
tocar la evidencia original, el instrumento se volvió a aplicar en un formulario alojado por un
tercero, que sella cada respuesta con la hora de su propio servidor: una marca de tiempo que el equipo
no controla, que es exactamente lo que faltaba. «Sellada» significa hora de servidor de Google, no un
sello criptográfico: la evidencia son nueve capturas, atadas al repositorio porque el código del
historial de Apps Script coincide línea por línea con \texttt{crear-formulario.gs}. El formulario no registra ninguna hora del lado del
cliente, por diseño. Se recogieron \textbf{15 respuestas} el 2026-09-18 entre las 11:36 y las 16:56, las
15 con fecha verificable y las 15 con \textbf{consentimiento individual explícito} --- lo que cierra
también la segunda objeción, la de que el consentimiento era solo una nota impresa. Resultado:
\textbf{media 52.83/100, desviación estándar 12.06, intervalo de confianza 95\,\% [46.16, 59.51]}
($n=15$). Según la escala de Bangor et al. (2009) sigue \textbf{por debajo} del promedio de la industria
(68 puntos): es un resultado desfavorable y se reporta como tal. Evidencia sin editar en
\texttt{docs/mediciones/sus/re-aplicacion/}, reproducible con \texttt{scripts/sus-ingesta.py}.

\textbf{Reclutamiento y composición.} El enlace se publicó en un servidor de \emph{Discord} en el que
ya estaban los estudiantes del curso, y se respondió de forma voluntaria y asíncrona a lo largo del
día: no hubo sesiones convocadas ni tarea guiada previa. Eso explica la dispersión de las marcas
temporales --- quince horas distintas repartidas entre las 11:36 y las 16:56, sin agrupamientos. La
composición declarada por los propios participantes es de 7 estudiantes, 5 docentes, 2 coordinadores y
1 administrador. Es \textbf{autoselección}: respondió quien quiso entre quienes vieron el enlace, sin
muestreo ni control de perfil, de modo que no puede descartarse que respondiera preferentemente quien
tenía mejor disposición hacia el sistema. Se declara como limitación
(Sección~\ref{sec:amenazas-validez}).

\textbf{Trazabilidad de la ronda.} Los tres hitos que fechan esta medición los sella Google, no el
equipo: el proyecto de Apps Script que construye el instrumento está guardado a las \textbf{11:16}
(historial del proyecto), el formulario se creó y compartió a las \textbf{11:22} (actividad de Drive)
y la primera respuesta llegó a las \textbf{11:36:04}. El archivo \texttt{crear-formulario.gs} se
\emph{comiteó} al repositorio a las 16:37, cinco horas más tarde: es la hora del commit, no la hora en
que se escribió, y no comitearlo en su momento fue un descuido real del equipo. Las capturas de las
tres interfaces están versionadas en
\texttt{docs/mediciones/sus/re-aplicacion/evidencia/}.

\textbf{Acceso concedido al evaluador.} Como una captura la aporta el propio equipo, se intentó además
transferir la \emph{propiedad} del formulario al evaluador. Google la rechaza mientras la cuenta de
destino tenga el almacenamiento lleno (\emph{``no se pueden compartir porque se ha superado el límite
de almacenamiento de Drive''}), de modo que esa parte no depende del equipo. Sí se concedió
\textbf{acceso de editor}, que según el propio diálogo de Google permite ``ver y eliminar las
respuestas del formulario'': con él, el evaluador puede comprobar por su cuenta y sin intermediación la
pestaña de respuestas, la hoja de cálculo vinculada y la fecha de creación en Drive. El proyecto de
Apps Script es un artefacto aparte y se comparte por separado si se solicita. Ambas capturas están
versionadas.

\textbf{Qué no resuelve esta ronda.} No es una re-aplicación a las 11 personas de las hojas en disputa:
de los 15 participantes, \emph{ninguno} declara haber respondido antes en papel (14 ``No'', 1 ``No
recuerdo''). Es una muestra nueva e independiente, y el origen de aquellas 11 hojas sigue igual de
abierto.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figuras/fig-sus-comparacion-rondas.png}
  \caption[Las tres mediciones del SUS]{Las tres mediciones del SUS con su intervalo de confianza del
  95\,\%. Los puntos claros son las respuestas individuales. La línea discontinua marca el promedio de
  la industria (68 puntos, Bangor et al.), por encima del cual no queda ninguna de las tres. Generada
  con \texttt{scripts/gen-figuras-sus.py} desde los CSV versionados.}
  \label{fig:sus-comparacion}
\end{figure}


\textbf{Lo que sí aporta sobre las hojas originales.} Las tres mediciones son estadísticamente
indistinguibles entre sí: papel con las 15 hojas (55.17), papel con solo las 4 de fecha verificable
(48.75) y el formulario del 18-sep (52.83), con prueba de Welch $t=0.519$, $p=0.608$ para la primera
comparación y $t=-1.278$, $p=0.220$ para la segunda
(Figura~\ref{fig:sus-comparacion}). Esos dos contrastes son la misma medición comparada contra dos
referencias, es decir una \emph{familia}, así que llevan corrección de Holm-Bonferroni con la misma
definición que la familia de pruebas de rendimiento (Sección~\ref{sec:evaluacion}): $p$ ajustados de
0.441 y 0.608 respectivamente, ninguno significativo.

\textbf{La conclusión no depende de las hojas retractadas.} Conviene dejarlo dicho porque la primera
fila de la comparación las incluye: si se descartan por completo, queda el contraste entre las 4 hojas
\emph{no} retractadas y el formulario, con $p$ ajustado de 0.441 --- tampoco significativo. La misma
conclusión se sostiene usando solo datos no retractados, de modo que quien rechace incluso ese uso
llega al mismo resultado. La corrección no cambia la conclusión --- se
aplica porque corresponde, no porque mueva el resultado a favor. Que una muestra distinta,
independiente y con fecha verificable caiga en el mismo rango es evidencia \emph{indirecta} de que los
puntajes de las hojas no son cifras inventadas para quedar bien; no dice nada sobre su fecha, y se
presenta con ese límite.

\textbf{Consistencia interna: $\alpha$ de Cronbach $= 0.599$.} El instrumento tiene una consistencia
interna \textbf{por debajo} de lo que se reporta habitualmente para el SUS (0.85--0.92 en Bangor et al.
2008 y Sauro 2011), y eso se declara junto a la cifra y no en una nota al pie. Tres precisiones. Primera:
\emph{no es un defecto de la ronda nueva} --- la ronda en papel da $\alpha = 0.619$, prácticamente lo
mismo, pese a ser otro soporte y otros participantes; si un $\alpha$ bajo indicara respuestas
fabricadas, no habría razón para que ambas coincidieran en él. Segunda: sin invertir los ítems pares el
$\alpha$ cae a 0.267, de modo que el salto a 0.599 al corregir la polaridad confirma que los
participantes \emph{sí} percibieron la alternancia del instrumento --- no marcaron en línea recta ---
pero no de forma lo bastante consistente como para que los diez ítems midan una sola dimensión.
Tercera, y es la que importa para leer el resultado: con $\alpha = 0.599$ y $n = 15$, el puntaje de
52.83 es un \textbf{indicador débil}, no una medición consolidada de usabilidad. Se mantiene porque es
lo que se midió. Reproducible con \texttt{scripts/sus-estadistica.py}.

Instrumento SUS de diez ítems. Las respuestas crudas de la ronda en papel están versionadas en
\texttt{docs/mediciones/sus/sus-respuestas.csv} (con una columna \texttt{fecha\_verificable}), con las 15
hojas escaneadas originales, sin modificar, en \texttt{docs/mediciones/sus/respuestas-crudas/}. Antes de
la ronda del 18-sep, el resultado de cierre se reportaba solo sobre las 4 respuestas con fecha
verificable (\textbf{media 48.75, DE 1.44, IC 95\,\% [46.45, 51.05]}, $n=4$: participantes G, H, K, Ñ).
Todos los puntajes se calculan con la fórmula estándar de Brooke y se verifican con \texttt{Python}
(\texttt{statistics}/\texttt{scipy}) directamente sobre los CSV; la implementación reproduce al dígito
los 15 puntajes de la ronda en papel. Una
...
\textbf{Cifra de cierre definitiva (2026-09-19, la que aplica para el umbral del 70\,\%), corrida limpia
de una sola sesión:} \textbf{82.13\,\% de líneas (4,054/4,936) y 73.52\,\% de ramas (1,491/2,028)} a
nivel global, sobre 823 pruebas automatizadas reales (\texttt{mvn clean test} sobre Postgres/Redis reales
en Docker, 0 fallos, 0 errores, una sola corrida desde cero, sin sesiones acumuladas), recalculado
directamente desde
\texttt{docs/mediciones/jacoco/2026-09-19-cierre-definitivo/jacoco.xml} (versionado en el
repositorio). Las dos corridas limpias previas (\texttt{2026-09-17-corrida-limpia-unica-sesion/} y
\texttt{2026-09-18-corrida-limpia-reproduccion/}, ambas 82.03\,\%/73.49\,\% sobre 4,897 líneas) se
conservan sin borrar: la diferencia con la cifra de cierre son las 4 líneas que agregaron las
correcciones posteriores al 18-sep. Desde esta misma ronda se agregó además una regla \texttt{jacoco:check} en \texttt{pom.xml}
(fase \texttt{test}, la que corre \texttt{./mvnw test} en CI) que hace fallar el build si la cobertura
cae del 70\,\% en líneas o en ramas --- pasó en esta corrida. Un snapshot anterior
(\texttt{docs/mediciones/jacoco/2026-09-17-cierre-examen-suspenso/}, 82.17\,\%/73.49\,\%) acumulaba 71
sesiones de ejecución en vez de una sola corrida limpia; se conserva sin borrar por trazabilidad, pero la
cifra que aplica es la de arriba. Nota real sobre el margen de ramas: unos 2.4 de los 3.49 puntos por
encima del umbral vienen de que \texttt{equals}/\texttt{hashCode} generados por Lombok en
\texttt{security/dto/*} (un paquete que la exclusión de JaCoCo del \texttt{pom.xml} no cubre, solo
excluye \texttt{ec/edu/uteq/presustentaciones/dto/**}) están bien probados --- sin ese efecto, la
cobertura de ramas sería 71.09\,\% (cálculo sobre la corrida del 2026-09-19), todavía por encima del umbral pero con un margen mucho más ajustado
(ver \texttt{docs/observaciones/OBSERVACIONES.md}, OBS-28, para el cálculo completo). Supera el umbral
del 70\,\% en líneas \emph{y} en ramas, con margen real (más de 12 y 1 puntos respectivamente, siendo
conservadores sobre el efecto de Lombok). El resto de esta sección documenta el historial de cómo se
llegó hasta acá --- se conserva sin reescribir por trazabilidad. \textbf{Corrección de una afirmación
anterior de este mismo párrafo:} decía que el bloque de \textbf{82.10\,\%/73.49\,\%} de más abajo (801
pruebas, 2026-09-15) "ya era, de hecho, la cifra correcta de cierre en el momento en que se escribió" la
guía del examen suspenso, y que por eso una lectura de esa guía podía citar 69.99\,\%/54.67\,\% "sin
llegar al párrafo que la supera". Eso es cronológicamente falso y así lo señaló el ingeniero: el commit
que la guía revisó (\texttt{f3d1ff4}, 13 de septiembre) es \emph{anterior} al commit que agregó el
párrafo del 82.10\,\% (\texttt{2b9ba89}, 15 de septiembre) --- verificado con
\texttt{git merge-base --is-ancestor f3d1ff4 2b9ba89}. La cifra del 82.10\,\% \emph{no existía todavía}
cuando se generó la guía; se agregó dos días después, en trabajo posterior. El párrafo con
\textbf{70.00\,\%/54.67\,\%} (2026-09-11) sí es una medición \emph{intermedia} real en esa historia, y
la guía la citó correctamente porque, en el commit que revisó, esa era la cifra vigente.
...
líneas para el 11-09 (0.53 puntos bajo el umbral, ramas sin cambio en 75\,\%) por el mismo efecto de
...
dos (\texttt{AuditController\#/tablas}, \texttt{TeacherController\#/disponibles} y \texttt{\#/paginado}).
Al mismo tiempo se corrigieron 8 fallos preexistentes de \texttt{SubmissionControllerTest} --- un
\texttt{NullPointerException} real por un mock de \texttt{PermissionService} faltante en el propio test, no
...
06-09. Los servicios se habían cerrado antes, el 2026-09-06, con 164 pruebas nuevas en
8 clases (\texttt{StudentService}, \texttt{TutorServiceImpl}, \texttt{RubricEvaluationServiceImpl},
\texttt{PanelistServiceImpl}, \texttt{SubmissionServiceImpl}, \texttt{TutoringServiceImpl},
\texttt{ScheduleServiceImpl}, \texttt{MinutesServiceImpl}), priorizando las clases con más ramas sin
...
sobre el commit de cierre del plazo. De los 31 controladores, \texttt{AppUserController} (78 líneas sin
ejercitar) y \texttt{BackupController} (40 líneas) eran los que más pesaban sin tener ningún test
dedicado. Se agregaron \texttt{AppUserControllerTest} (23 pruebas, cubre el control de propiedad real vía
\texttt{esAppUserActual}/\texttt{esAppUserActualOAdmin}, no sólo el permiso \texttt{USUARIOS\_GESTIONAR})
y \texttt{BackupControllerTest} (19 pruebas, los 17 endpoints protegidos por \texttt{BACKUPS\_GESTIONAR}
a nivel de clase), llegando el 2026-09-13 a \textbf{79.01\,\% de líneas (990/1,253) y 80.34\,\% de ramas (286/356)} ---
...
fail-closed de RNF-04 cuando Redis no responde). Esa primera pasada (entre el 13-09 y el 15-09) dejó 71.90\,\% de ramas --- por
encima del umbral pero al filo para el gusto del equipo, así que se hizo una segunda pasada sobre lo
que quedaba: \texttt{StatusLiveController} y \texttt{ProposalController} (0\,\% cada uno,
sin ningún test), y \texttt{EmailService}/\texttt{AuditService} (0\,\% cada uno). Resultado:
\textbf{82.13\,\% de líneas (4,054/4,936) y 73.52\,\% de ramas (1,491/2,028)} a nivel global (823
pruebas, 0 fallos).
...
número de tests no cambió --- sino de que \texttt{PreDefenseApplicationTests} dejó de ser un
...
IDOR (\texttt{SubmissionAccessServiceTest}, 8 tests, y casos permitido/IDOR/admin en los 4 servicios
...
(\texttt{ResourceDegreeController}, \texttt{EvaluationPanelistController}). Además, el proyecto ya
...
\texttt{PreDefenseApplicationTests} y \texttt{TopicProposedRepositoryIntegrationTest} (8 tests) ---
...
\bottomrule
\end{tabular}
\caption{Lighthouse, media de 3 corridas por perfil (2026-09-17, contra el despliegue público en Railway).}
\end{table}
